...
const PRODUCTS_SET_1: Product[] = [
  { id: "p-asus-zenbook-14", name: "Notebook ASUS ZenBook 14", price: 32900, image: "/images/Notebook 1.png", brand: "ASUS" },
  { id: "p-lenovo-x1", name: "Lenovo ThinkPad X1 Carbon", price: 48900, image: "/images/Notebook 2.png", brand: "Lenovo" },
  { id: "p-dell-xps-13", name: "Dell XPS 13", price: 42500, image: "/images/Notebook 3.png", brand: "Dell" },
  { id: "p-apple-air-m2", name: "MacBook Air M2", price: 39900, image: "/images/Notebook 4.png", brand: "Apple" },
  { id: "p-hp-spectre-x360", name: "HP Spectre x360", price: 45500, image: "/images/Notebook 5.png", brand: "HP" },
];

const PRODUCTS_SET_2: Product[] = [
  { id: "p-asus-rog-g14", name: "ASUS ROG Zephyrus G14", price: 56900, image: "/images/Tablet 1.png", brand: "ASUS" },
  { id: "p-lenovo-yoga-7", name: "Lenovo Yoga Slim 7", price: 38900, image: "/images/Tablet 2.png", brand: "Lenovo" },
  { id: "p-dell-inspiron-14", name: "Dell Inspiron 14", price: 29900, image: "/images/Tablet 3.png", brand: "Dell" },
  { id: "p-apple-pro-14", name: "MacBook Pro 14 (M3)", price: 69900, image: "/images/Tablet 4.png", brand: "Apple" },
  { id: "p-hp-envy-13", name: "HP Envy 13", price: 31900, image: "/images/Tablet 5.png", brand: "HP" },
];

const PRODUCTS_SET_3: Product[] = [
  { id: "p-asus-vivobook-s15", name: "ASUS Vivobook S15", price: 27900, image: "/images/Smartwatch 1.png", brand: "ASUS" },
  { id: "p-lenovo-ideapad-5", name: "Lenovo IdeaPad 5", price: 24900, image: "/images/Smartwatch 2.png", brand: "Lenovo" },
  { id: "p-dell-latitude-7440", name: "Dell Latitude 7440", price: 45900, image: "/images/Smartwatch 3.png", brand: "Dell" },
  { id: "p-apple-air-m3", name: "MacBook Air M3", price: 42900, image: "/images/Smartwatch 4.png", brand: "Apple" },
  { id: "p-hp-pavilion-15", name: "HP Pavilion 15", price: 23900, image: "/images/Smartwatch 5.png", brand: "HP" },
];

const PRODUCT_SETS: Product[][] = [PRODUCTS_SET_1, PRODUCTS_SET_2, PRODUCTS_SET_3];
const ALL_PRODUCTS: Product[] = [...PRODUCT_SETS[0], ...PRODUCT_SETS[1], ...PRODUCT_SETS[2]];
...
function deriveNoun(raw: string): Noun {
  ...
  if (/(smart ?watch|apple ?watch|galaxy ?watch|watch)/.test(q)) return "Smartwatch";
  if (/(tablet|ipad|galaxy ?tab|\btab\b)/.test(q)) return "Tablet";
  if (/(notebook|laptop|macbook|zenbook|xps|thinkpad|pavilion|inspiron|vivobook)/.test(q)) return "Notebook";
...
function rankBrands(input: string, brands: BrandName[]): BrandName[] {
  const q = normalize(input);
  if (!q) return brands;
  const starts = brands.filter((b) => BRAND_KEYWORDS[b].some((k) => k.startsWith(q)));
  const contains = brands.filter((b) => !starts.includes(b) && BRAND_KEYWORDS[b].some((k) => k.includes(q)));
...
function buildBrandAutocomplete(input: string, noun: Noun): BrandOption[] {
  ...
  return ordered.slice(0, MAX_BRANDS).map((b) => ({ brand: b, query: BRAND_KEYWORDS[b][0] }));
}
...
function rankTextOptions(input: string, options: string[]): string[] {
  ...
  const contains = options.filter((o) => !starts.includes(o) && normalize(o).includes(q));
  return [...starts, ...contains];
}
function buildCategorySuggestions(input: string, noun: Noun): SimpleOption[] {
  ...
  return ranked.slice(0, MAX_CATEGORY).map((label) => ({ label, query: label }));
}
...
const ProductCard = memo(({ product }: { product: Product }) => (
  <div className="card-outer-bg card-outer-shadow w-[220px] max-w-[220px] shrink-0 rounded-[25px] p-[1px] snap-start">
...
export default function SearchEngineAI() {
  ...
  const [displayProducts, setDisplayProducts] = useState<Product[]>(PRODUCTS_SET_1.slice(0, MAX_PRODUCTS));
  const initialBrandGuesses = useMemo<BrandOption[]>(() => buildBrandAutocomplete("", "Notebook"), []);
  const [displayBrands, setDisplayBrands] = useState<BrandOption[]>(initialBrandGuesses.slice(0, MAX_BRANDS));
  ...
  useEffect(() => {
    const update = () => setGhostPadLeft((startSlotRef.current?.offsetWidth ?? 56) + 16);
    update();
  ...
  const recalcConstraints = useCallback(() => {
    ...
    if (current < nextMin || current > 0) {
      animate(x, clamp(current, nextMin, 0), { type: "spring", bounce: 0.2, duration: 0.5 });
    }
  }, [x]);
  ...
  const timersRef = useRef<{ typing: Interval | null; pause: Timeout | null }>({ typing: null, pause: null });

  const clearAllTimers = useCallback(() => {
    if (timersRef.current.typing) { clearInterval(timersRef.current.typing); timersRef.current.typing = null; }
    if (timersRef.current.pause) { clearTimeout(timersRef.current.pause); timersRef.current.pause = null; }
  }, []);

  // base noun & dataset
  const currentNoun = useMemo<Noun>(() => (text.trim() ? deriveNoun(text) : "Notebook"), [text]);
  const baseNoun: Exclude<Noun, "สินค้า"> = isBaseNoun(currentNoun) ? currentNoun : "Notebook";
  ...
  const inlineRemainder = useMemo(() => {
    ...
    if (isTypingNounPrefix && noun.startsWith(q) && baseNoun.length > raw.length) {
  ...
  useEffect(() => {
    ...
    setDisplayProducts(() => {
      ...
      if (basis.length < MAX_PRODUCTS) {
        const padPool = fallback.filter((f) => !basis.some((b) => b.id === f.id));
  ...
  useEffect(() => {
    const defaultGuess = buildBrandAutocomplete("", currentNoun).slice(0, MAX_BRANDS);
    setDisplayBrands((prev) => {
      const next = (brandSuggestions.length ? brandSuggestions : defaultGuess).slice(0, MAX_BRANDS);
      if (next.length < MAX_BRANDS) {
        const pad = prev.filter((b) => !next.find((n) => n.brand === b.brand)).slice(0, MAX_BRANDS - next.length);
        return [...next, ...pad];
      }
      return next;
    });
            ...
            <div className="card-outer-bg card-outer-shadow relative rounded-full p-[1px]">
              <div className="card-inner-bg card-inner-blur rounded-full relative">
                ...
                {(text || inlineRemainder) && (
                  <div className="pointer-events-none absolute inset-0 flex items-center z-[1]">
                    <div
                  ...
                  <ScrollShadow orientation="vertical" className="max-h-[450px] overflow-x-hidden" hideScrollBar>
                    <div className="grid gap-6 px-4 pt-2 pb-4 md:grid-cols-1">
                      ...
                      <div className="md:order-1 md:col-span-2">
                        ...
                        {/* Category — คง 2 */}
                        <div className="mt-5 text-xs tracking-wider text-white/40">{UI.Category}</div>
                        <ul className="mt-2 space-y-1">
                          {categorySuggestions.slice(0, MAX_CATEGORY).map((c) => (
                            <li key={c.label}>
                              <button
                        ...
                        <div className="mt-5 text-xs tracking-wider text-white/40">{UI.productHeader}</div>
                      </div>
                      ...
                      <div className="md:order-2 md:col-span-3">
                        ...
                        <div className="-mx-6 md:-mx-8 px-6 md:px-8">
                          <div ref={viewportRef} className="relative h-[240px] w-full overflow-hidden">
                            <motion.div
                              ref={trackRef}
                        ...
                        {/* Spec — คง 2 */}
                        <div className="mt-5 text-xs tracking-wider text-white/40">{UI.Specification}</div>
                        <ul className="mt-2 space-y-1">
